...
int CPARSERV1::Pattern_Match(const char* string, const char* pattern, ...)
{
...
int StripChars(const char* string_in, char* string_out, const char* strip_chars, int case_on)
{
	...
	while (1)
	{
	 ...
	 // check for pattern?  
		if (parser.Pattern_Match(parser.buffer, "['center'] [f] [f] [f]"))
		{
			...
			Print_Mat_4X4(&mat_local, const_cast <char*>("Local COB Matrix:"));
	...
	while (1)
	{
	 ...
	 // check for pattern?  
		if (parser.Pattern_Match(parser.buffer, "['Transform']"))
		{
			...
			Print_Mat_4X4(&mat_world, const_cast <char*>("World COB Matrix:"));
